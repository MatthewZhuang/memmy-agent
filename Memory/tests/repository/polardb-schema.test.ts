...
describe("repository PolarDB schema contract", () => {
  it("publishes migration SQL for the memories table and runtime support tables", () => {
    ...
    expect(sql).toContain("CREATE TABLE IF NOT EXISTS span_clusters");
    expect(sql).toContain("goal_centroid JSONB");
    expect(sql).toContain("policy_centroid JSONB");
    expect(sql).toContain("CREATE TABLE IF NOT EXISTS span_cluster_members");
    expect(sql).toContain("UNIQUE (span_id, algorithm_version)");
    ...
    expect(sql).toContain("target_kind TEXT NOT NULL CHECK (target_kind IN ('trace', 'span', 'policy', 'world_model', 'skill'))");
    expect(sql).toContain("vector_field TEXT NOT NULL CHECK (vector_field IN ('vec_summary', 'vec_action', 'vec', 'vec_goal', 'vec_policy'))");
