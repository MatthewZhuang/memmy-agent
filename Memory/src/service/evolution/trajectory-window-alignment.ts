...
/**
 * Align two fixed-length Step-vector sequences without allowing their order to
 * change. The diagonal band bounds both runtime and the amount of positional
 * drift that can be accepted as the same procedure.
 */
...
export function selfBandedMonotonicMatch(
  ...
  validateConfig(config);
  if (vectors.length !== config.scale) {
    throw new Error(`self banded match expected scale ${config.scale}, got ${vectors.length}`);
  }
  const pairs = vectors.map((_, index) => ({
    leftIndex: index,
    rightIndex: index,
    similarity: 1
  }));
...
/** Returns -1 for incompatible or zero vectors, matching the mining gate semantics. */
